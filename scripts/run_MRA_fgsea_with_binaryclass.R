...
#indices = which(mra$padj<0.05 & abs(mra$NES)>1.0);
#mra_table <- data.frame(topmr=mra$pathway[indices],padj=mra$padj[indices],NES=mra$NES[indices]);
#write.table(mra_table,file=paste0(outputpath,"/Adjacency_Matrix/",sample_name,"topMRs_FGSEA_BC_NES_1.csv"),col.names=T,row.names=F)
