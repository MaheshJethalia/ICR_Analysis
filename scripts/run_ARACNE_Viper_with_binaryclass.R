...
#=======================================================================================
net <- read.table(gzfile(paste0(outputpath,"/Adjacency_Matrix/Final_Adjacency_Matrix_v2.csv.gz")),header=TRUE,sep=" ")
...
vpres_high <- viper(vpsig_high, viper_regulons, minsize=10, cores=1, verbose = TRUE)
vpsig_low <- viperSignature(D_low,D_medium,method="zscore",per=1000,seed=123,cores=48,verbose=TRUE)
vpres_low <- viper(vpsig_low, viper_regulons, minsize=10, cores=1, verbose = TRUE)

vpres_all <- as.matrix(cbind(vpres_high,vpres_low))

#Perform limma to get the most differential gene sets
#=======================================================================================
icr_info <- c(rep("ICR_High",length(high_indices)),rep("ICR_Low",length(low_indices)))
design <- model.matrix(~ factor(icr_info))
fit <- lmFit(vpres_all, design)
fit <- eBayes(fit)
DEgeneSets <- topTable(fit, coef=2, number=Inf,
                       p.value=0.05, adjust="fdr")
save(DEgeneSets,file=paste0(outputpath,"/Adjacency_Matrix/",filename,"_All_Diff_MR_Info_Viper_BC_NES_1.Rdata"))
...
# #=================================================================================================
# pdf(paste0(outputpath,"/Images/",filename,"_Activity_Viper_BC_NES_1.pdf"),pointsize=9,height=10,width=10)
