...
D <- D[,filter_samples];
jpeg(filename="Results/Figures/Supp_Figure1.jpg",width=900, height=480, units="px", pointsize=12)
...
hc.cols <- hclust(dist(t(D1)),method="ward.D2")
hc.rows <- hclust(dist(D1),method="ward.D2")

pdf(paste0("Results/Figures/RNA_Seq_Figure_1A.pdf"),width=12,height=10,pointsize = 16)
par(bg="white")
par(fg="black",col.axis="black",col.main="black",col.lab="black", cex.main=2.0)
heatmap.3(D1[hc.rows$order,hc.cols$order], Rowv = NULL, Colv = NULL, col = bluered(50), scale="none", main= "RNA-Seq Expression Data",
          xlab = "TCGA Samples", ylab = "23,216 Genes ", labRow = FALSE, labCol = FALSE, dendrogram = "none", cexRow = 6, cexCol = 6,
...
im1 <- readPicture("Results/Figures/RNA_Seq_Figure_1A.svg")
g1 <- pictureGrob(im1)

df1 <- data.frame(distance = 1,
                  number = 1)
#g1 <- ggplot(data = df1, aes(x = distance, y = number)) + 
#      annotation_custom(pictureGrob(im1, width = unit(1,"npc"), height = unit(1,"npc"))) + 
#      theme_void() + coord_fixed(ratio=1.25)
...
im2 <- readPicture("Results/Figures/BLCA_GRN_Figure_1B.svg")
g2 <- pictureGrob(im2)
...
#jpeg(paste0("Results/",filename,"/Images/",filename,"_Corr_Matrix_Ordered.jpg"),width=720,height=480,units = "px", pointsize = 12)
#heatmap.3(corr_matrix, Rowv = NULL, Colv = NULL, col = bluered(50), scale="none", main= "TF-target Correlation Matrix",
#          xlab = "Target Genes", ylab = "TFs", labRow = FALSE, labCol = FALSE, dendrogram = "none", colsep=c(1,ncol(corr_matrix)), 
#          sepcol = "black", rowsep = c(1,nrow(corr_matrix)),
#          key = TRUE, density.info = "none", KeyValueName = "Pearson Correlation",
#          margins = c(2,2), useRaster = TRUE)
#dev.off()

#im3 <- readJPEG(paste0("Results/",filename,"/Images/",filename,"_Corr_Matrix_Ordered.jpg"))
#g3 <- ggplot(data = df1, aes(x = distance, y = number)) + 
#  annotation_custom(rasterGrob(im3, width = unit(1,"npc"), height = unit(1,"npc")),-Inf, Inf, -Inf, Inf) + 
#  theme_void() + coord_fixed(ratio=0.66)
...
#===================================================================================
im4 <- readPicture("Results/Figures/Toy_TF_Regulon_Figure_1C.svg")
g4 <- pictureGrob(im4)
...
pdf("Results/Figures/BLCA_TF_Activity_Figure_1D.pdf",width=12,height=12,pointsize=16)
par(bg="white")
...
#===================================================================================
im5 <- readPicture("Results/Figures/BLCA_TF_Activity_Figure_1D.svg")
g5 <- pictureGrob(im5)
...
#========================================================================================
#jpeg(paste0(outputpath,"/Images/",sample_name,"GeneRanks_FGSEA_BC.jpg"),pointsize=12,height = 600, width = 600)
pdf("Results/Figures/BLCA_GeneRanks_FGSEA_Figure_1E.pdf",pointsize=18,height = 10, width = 10)
par(bg="white")
par(fg="black",col.axis="black",col.main="black",col.lab="black", cex.main=2.0, cex = 2.5)
plotGseaTable(regulon_sets[topPathways], sorted_pheno_t, fgseaRes, gseaParam = 0.5)
dev.off()

im6 <- readPicture("Results/Figures/BLCA_GeneRanks_FGSEA_Figure_1E.svg")
g6 <- pictureGrob(im6)
...
#=================================================================================================
pdf("Results/Figures/BLCA_TopMR_Activity_Figure_1F.pdf",height=10,width=12,pointsize=16)
par(bg="white")
par(fg="black",col.axis="black",col.main="black",col.lab="black", cex.main=2.0, cex = 2.5)
...
im7 <- readPicture("Results/Figures/BLCA_TopMR_Activity_Figure_1F.svg")
g7 <- pictureGrob(im7)
...
ggsave(filename = paste0("Results/Figures/Figure1.pdf"),device = pdf(), plot = g_final, dpi = 300, width = 12, height= 10.5, units = "in" )
dev.off()
