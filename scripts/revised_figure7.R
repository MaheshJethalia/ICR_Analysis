...
icr_neutral_cancers <- setdiff(setdiff(setdiff(list.files(RGBM_path),icr_disabled_cancers),icr_enabled_cancers),c("Others","Assembler_Panca_Normalized","ARACNE",
                                                                                                                  "PanCancer","PRECOG"))
...
#save(Activity_Matrix,all_cold_mrs,all_hot_mrs,cancer_info_vector,icr_type_info_vector,file="Results/PanCancer/ICR_Neutral_Activity_Revised(v2)_Info.Rdata")
...
write.table(activity_df,"../Results/Revised_Text_Results/Diff_MRS_Activity_ICR_Neutral_Revised(v2)_Table_12.csv",row.names=T,col.names=T,quote=F,sep=",")
